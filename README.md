...
- オプションページから次の項目をカスタマイズ可能
  - 表示する/しないボタンの切り替え
  - 表示スタイル（アイコンのみ / アイコン+テキスト）
  - グルーピング（個別表示 / まとめて表示）
  - 並び順（上下ボタンで変更）
  - 新しいタブで開くかどうか
  - 存在チェックの有効/無効と DeepWiki の判定方法（ページ解析 / 公式 MCP エンドポイント）
- 任意の URL テンプレートを使った **カスタムサービス** の追加にも対応（最大 20 件）
...
Chrome ウェブストアで「Kongyo GitHub Custom」を検索してインストールしてください。
